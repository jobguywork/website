  ...
  <div class="content-tab rtl">
    ...
    <div class="title layout-h layout-justified">
      <div class="layout-h layout-center-center">
        ...
        <h2 class="label">
          تجربه های کاری در <span>{{ company.name }}</span>
        </h2>
      </div>
      <el-button size="mini" type="primary" @click="addReview">
        ثبت تجربه کاری
      </el-button>
    </div>
    <!-- /title -->
    <div v-if="reviews.data.length > 0" class="reviews mt-10">
      <ul>
        <Review
      ...
      <client-only v-if="reviews.total > 0">
        <infinite-loading
          :identifier="reviews.infiniteId"
    ...
    <Empty v-else class="mt-20">
      <p>
        اولین نفر باش که تجربه کاریت، در {{ company.name }} را به اشتراک می
        گذاری.
      </p>
      <el-button size="mini" type="primary" @click="addReview">
        ثبت تجربه کاری
...
export default {
  ...
  methods: {
    addReview() {
      this.$router.push(`/review/create/${this.company.company_slug}/`)
    },
    async infiniteHandler(state) {
      await this.$store.dispatch('company/review/getReviews', {
